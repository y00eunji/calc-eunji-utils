...
function isOnlyNumber(expression: string): boolean {
...
// 괄호의 짝이 맞지 않는 경우
function hasUnmatchedParentheses(expression: string): boolean {
    let openBrackets = 0;
    for (const char of expression) {
        if (char === '(') openBrackets++;
        if (char === ')') openBrackets--;
        if (openBrackets < 0) return true;  // 닫는 괄호가 더 많으면 실패
    }
